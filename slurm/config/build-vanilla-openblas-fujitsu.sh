#!/usr/bin/env bash
#SBATCH --job-name=build-vanilla-openblas-fujitsu
...
BUILD_DIR="${BUILD_DIR:-$REPO_DIR/build-blas/openblas-fujitsu}"
...
if [[ -n "${EBROOTOPENBLAS:-}" ]]; then
  CMAKE_ARGS+=(
    -DCMAKE_PREFIX_PATH="$EBROOTOPENBLAS"
    -DCMAKE_BUILD_RPATH="$EBROOTOPENBLAS/lib"
  )
fi
...
echo "[INFO] BLAS module: $OPENBLAS_MODULE"
cmake "${CMAKE_ARGS[@]}"
...
echo "[OK] OpenBLAS/Fujitsu vanilla build: $BUILD_DIR/bin/llama-server"
