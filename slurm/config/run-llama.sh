...
if [[ -z "${MODEL_PATH:-}" ]]; then
  ...
  # Keep a sensible default for the error message when the file is not found.
  if [[ -z "$MODEL_PATH" ]]; then
...
if [[ ! -x "$SERVER_BIN" ]]; then
  ...
  echo "Submit build job first: sbatch slurm/config/build-llama.sh" >&2
  exit 1
fi
